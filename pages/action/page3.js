...
export const getStaticProps = async () =>{
    ...
    let actionMovies = data.items.filter(action=>action.genre_ids.find(i=>i==28)).slice(40)
    return {
...
let Page3=({movieList})=>{
    return <>
          ...
          <div className={movie.pages} ><Link href={'page1'}><span>1</span></Link><Link href={'page2'}><span>2</span></Link><span style={{color:'blue',fontSize:'1.3em'}}>3</span></div>
      </div>
    </>
}
