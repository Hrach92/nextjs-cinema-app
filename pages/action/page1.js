...
export const getStaticProps = async () =>{
    ...
    let actionMovies = data.items.filter(action=>action.genre_ids.find(i=>i==28)).slice(0,20)
    return {
  ...
  let Action = ({movieList}) =>{
      ...
      <div className={movie.container}>
          ...
          <div className={movie.pages}><span style={{color:'blue',fontSize:'1.3em'}}>1</span><Link href={'page2'}><span>2</span></Link><Link href={'page3'}><span>3</span></Link></div>
      </div>
      </>
  }
