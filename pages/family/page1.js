...
export const getStaticProps = async () =>{
    const response = await fetch('https://api.themoviedb.org/3/list/55?api_key=c1fe0a5e08d1bf80be453d4b47b27699')
    ...
    let animationMovies = data.items.filter(action=>action.genre_ids.find(i=>i==10751))
    return {
      props:{movieList:animationMovies}
    }
  }

  let Family = ({movieList}) =>{
      debugger
    return <>
